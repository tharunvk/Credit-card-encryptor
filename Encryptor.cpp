...
int encryption_func(int num_to_encrypt, string * subkeys, int flag){
  ...
  int sbox[8][4][16] =             // sboxes
    ...
    //adding tweak
    if(flag==0){
      num_to_encrypt = num_to_encrypt + tweak;
       if(num_to_encrypt>=1000000){
    	num_to_encrypt= num_to_encrypt - 1000000;
       }
    }
    string bits_to_encrypt = dec2bin(num_to_encrypt);


    //padding bits to rhs
    int pad = 20 - bits_to_encrypt.size();
	string strpad = "";
	for (int i = 0; i < pad; ++i)
	{
		strpad += '0';
	}
	bits_to_encrypt = strpad + bits_to_encrypt;

	string lhs, rhs;

	lhs = bits_to_encrypt.substr(0, 10);
	rhs = bits_to_encrypt.substr(10, 10);

	string zero_pad = "0000000000000000000000";

	//Iterating encryption algorithm 16 times
	...
	for (int i = 0; i < 16; i++) {
        ...
        //swappinh lhs and rhs
		string temp = rhs;
		rhs = lr_xor;
		lhs = temp;		
	}
    string ciphertext = rhs + lhs;
    return bin2dec(ciphertext);

}
int main() {
	string ccn;
	cout<<" Enter Credit card numer : ";  //Enter ccn
	cin>>ccn;
  if(ccn.size()<16){
    cout<<"Please enter valid 16 digit number"<<endl;
    exit(0);
  }
	int num_to_encrypt = get_six_digits(ccn);
	cout<<"Num to encrypt :"<<num_to_encrypt<<endl;  //getting 6 digits
    tweak = stoi(ccn.substr(0,2)+ccn.substr(ccn.size()-4,4));

	string key = "0110011001010100011010100101011101101110010110100111001000110100";
	string subkeys[16];
	subkeys_gen(key, subkeys);  //generating keys

    int k=0;
    string reversed_subkeys[16];
	for (int i = 15; i >= 0; i--)
	{
	...
	do{
		num_to_encrypt = encryption_func(num_to_encrypt,subkeys,0);   //the encryption function
		
	}while(num_to_encrypt>=1000000);

    cout<<"Encrypted number : "<<num_to_encrypt<<endl;   //encrypted number

    string enc_num = to_string(num_to_encrypt);
    string final_string = ccn.substr(0,6)+enc_num+ccn.substr(ccn.size()-4,4);
    //int enc_ccn = stoi(final_string);
	cout<<"Encrypted Credit card number : "<<final_string<<endl;   //encrypted ccn
  int getch;
  cin>>getch;
	return 0;
}
